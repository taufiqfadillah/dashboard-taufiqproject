!(function (e) {
  e(document).ready(function () {
    ...
    e('.add-task-btn').click(function () {
      ...
      e(this).toggleClass('hide'), e('.new-task-wrapper').toggleClass('visible'), e('#new-task').focus(), e('body').animate({ scrollTop: s }, 1e3);
    }),
...
$('#add-task').click(function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      const newTaskHtml = $('#task-template').html();
      newTaskHtml.replace('Task Label', task);
      $('#todo-list').append(newTaskHtml);
      $('#new-task').val('');
...
$('#todo-list').on('click', '.complete-btn', function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      const taskLabel = $(this).closest('.task').find('.task-label').text();
      showNotification(taskLabel, 'marked as complete.', 'success');
      $(this).toggleClass('completed');
      const badge = $(this).closest('.task').find('.badge');
      badge.removeClass('badge-light-danger').addClass('badge-light-success');
      badge.text('Done');
    },
    error: function (error) {
      console.error(error);
...
$('#todo-list').on('click', '.delete-btn', function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      const taskLabel = $(this).closest('.task').find('.task-label').text();
      showNotification(taskLabel, 'has been deleted.');
...
function showNotification(task, action, type = '') {
  const notificationPopup = $('.notification-popup');
  notificationPopup.find('.task').text(task);
  notificationPopup.find('.notification-text').text(action);
  notificationPopup.removeClass('hide success').addClass(type);
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      data.todos.forEach((todo) => {
        const taskHtml = `
            <li class="task" data-task-id="${todo._id}">
              <!-- ... kode task HTML ... -->
            </li>`;
